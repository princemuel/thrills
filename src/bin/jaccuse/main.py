...
while True:  # Main game loop.
    ...
    if response == "J":  # Player accuses this suspect.
        ...
        if thePersonHere == culprit:
            ...
            print(
                "Good job! You solved it in {} min, {} sec.".format(
                    minutesTaken, secondsTaken
                )
            )
            sys.exit()
        else:
        ...
        if thePersonHere not in zophieClues:
            ...
            if (
                zophieClues[thePersonHere] not in knownSuspectsAndItems
                and zophieClues[thePersonHere] not in PLACES
            ):
        ...
        if thingBeingAskedAbout in (thePersonHere, theItemHere):
            ...
            print(
                '  They give you this clue: "{}"'.format(
                    clues[thePersonHere][thingBeingAskedAbout]
                )
            )
            # Add non-place clues to the list of known things:
            if (
                clues[thePersonHere][thingBeingAskedAbout] not in knownSuspectsAndItems
